        ...
        getDataAsync: function(keys) {
            return new Promise(function(resolve, reject){
                ...
                else
                {
                    ...
                    data = JSON.parse(data);
                    if (data === null)  // TEMP: Loads previous games before storaging scoping change
                    {
...
LibMiniclip.Log = function(message)
{
    console.log(message);
}

LibMiniclip.LogError = function(message)
{
    console.log(message);
}
...
//
// options
LibMiniclip.prototype.Init = function(options)
{
    LibMiniclip.Log(">>>> Miniclip: Init");
    window.mcAsyncInit = function() {
        MC.init({
            app_id: 0,
            language: 'en',
            modules: ['currency','social']
        });
        MC.resize();
    };
    (function(d, s, id){
        var js, mcjs = d.getElementById('gamecanvas');  // miniclip-js
...
LibMiniclip.prototype.InitUser = function(options)
{
}

LibMiniclip.prototype.addSupportedAPI = function(type)
{
    if (type === "user")
    {
        FBInstant.supportedAPIs.push("getLocale");
...
// LOGIN AND ACCOUNTS
//
LibMiniclip.prototype.Login = function(allow_anonymous, done_cb)
{
    LibMiniclip.Log(">>>> Miniclip: Login");
    if (this.loggedIn)
    {
        if (done_cb !== undefined)
            done_cb(null, null);
...
//            document.write(response.auth.access_token);
            LibMiniclip.Log(">>>> Miniclip: Logged in");
            LibMiniclip.Log(response);
            this.loggedIn = true;
            if (done_cb !== undefined)
                done_cb(null, null);
        });
    ...
    {
        LibMiniclip.Log(">>>> Miniclip: Anonymous login");
        if (done_cb !== undefined)
            done_cb(null, null);
    }
};
...
// PROFILE
//
LibMiniclip.prototype.GetProfileData = function()
{
    if (!this.loggedIn)
    {
        return null;
    }
    return new UserService.Player("1", "Guest", null, null, LibUtils.GetLocale());
}

LibMiniclip.prototype.GetFriends = function(done_cb)
{
    if (done_cb !== undefined)
        done_cb([]);
}
